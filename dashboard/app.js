...
function render() {
  ...
  panels.innerHTML = `
    ...
    ${metricPanel("Browser RSS", sampledResources.length ? formatBytes(totalRssBytes) : "—", `${sampledResources.length} of ${state.sessions.length} session process trees measured.`)}
    ${metricPanel("Browser CPU", sampledResources.length ? formatDurationMs(totalCpuTimeMs) : "—", "Cumulative CPU time for currently measured browser process trees.")}
...
function renderSessionResources(resources) {
  if (!resources || !resources.available) {
    ...
    return `<div class="session-resources" data-state="unavailable"><strong>Resource sample unavailable</strong><span>—</span><small>${escapeHtml(reason)}</small></div>`;
  }
  ...
  return `<dl class="session-resources" data-state="${stateValue}">
    ...
    <div><dt>Processes</dt><dd>${escapeHtml(resources.processCount ?? "—")}</dd></div>
