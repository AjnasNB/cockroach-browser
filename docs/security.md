...
Cockroach Browser intentionally excludes CAPTCHA or access-control bypass, covert stealth or cloaking, automatic discovery of ambient browser cookies or profiles, and public unauthenticated server binding.Use explicit runtime-owned profiles, reviewed browser providers, authenticated loopback or TLS transport, human challenge handoff, and Maqam-ready exact approval for consequential operations. These are product invariants, not missing browser features.
